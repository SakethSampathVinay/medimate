    ...
    <AppLayout>
      <Tabs defaultValue="doctors" className="space-y-4">
          ...
          {filteredDoctors.length > 0 ? (
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              {filteredDoctors.map(doctor => (
                <Card key={doctor.id} className="flex flex-col overflow-hidden shadow-md hover:shadow-xl transition-shadow dark:bg-card">
                  <CardHeader className="flex-row items-start gap-4">
                    ...
                    <div className="flex-grow">
                      <CardTitle className="font-headline text-xl text-card-foreground">{doctor.name}</CardTitle>
                      ...
                      <div className="flex items-center text-xs text-amber-500 dark:text-amber-400 mt-1">
                        <StarIcon className="mr-1 h-3 w-3 fill-amber-500 dark:fill-amber-400" /> {doctor.rating.toFixed(1)}
                      </div>
                  ...
                  <CardFooter className="mt-auto">
                    <Button className="w-full bg-primary hover:bg-primary/90 text-primary-foreground" onClick={() => handleViewProfileBook(doctor)}>
                      View Profile & Book
                    </Button>
                  </CardFooter>
                </Card>
            ...
            <Card className="dark:bg-card">
              <CardContent className="text-center py-12">
                ...
                <p className="text-lg font-medium text-card-foreground">No doctors found</p>
        ...
        <TabsContent value="packages" className="space-y-8">
           <Card className="dark:bg-card">
            <CardHeader>
              <CardTitle className="font-headline text-2xl text-card-foreground">Predefined Health Checkup Packages</CardTitle>
            ...
            <CardContent className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              {mockHealthCheckupPacks.map(pack => (
                <Card key={pack.id} className="flex flex-col overflow-hidden shadow-md hover:shadow-xl transition-shadow dark:bg-card">
                  ...
                  <CardHeader>
                    <div className="flex items-center mb-2">
                      ...
                      <CardTitle className="font-headline text-xl text-card-foreground">{pack.name}</CardTitle>
                    </div>
                  ...
                  <CardContent className="flex-grow">
                    <h4 className="font-medium text-sm mb-1 text-card-foreground">Tests Included:</h4>
                    <ul className="list-disc list-inside text-xs text-muted-foreground space-y-0.5">
                      {pack.testsIncluded.map(test => <li key={test}>{test}</li>)}
                    </ul>
                  </CardContent>
                  <CardFooter>
                    <Button className="w-full bg-accent hover:bg-accent/90 text-accent-foreground" onClick={() => handleBookPackage(pack)}>
      ...
      {selectedDoctorForModal && (
        <Dialog open={isTimeSlotModalOpen} onOpenChange={setIsTimeSlotModalOpen}>
          <DialogContent className="sm:max-w-md dark:bg-card">
            <DialogHeader>
              <DialogTitle className="font-headline text-xl text-card-foreground">Book Appointment with {selectedDoctorForModal.name}</DialogTitle>
              <DialogDescription className="space-y-1">
                <div>{selectedDoctorForModal.specialty} - {selectedDoctorForModal.degree}</div>
                <div className="font-semibold text-primary">Consultation Fee: ₹{selectedDoctorForModal.fees}</div>
            ...
            <div className="grid gap-4 py-4">
              ...
              {modalSelectedDate && selectedDoctorForModal.availability[format(modalSelectedDate, "yyyy-MM-dd")]?.length > 0 ? (
                <div className="space-y-2">
                  <h4 className="font-semibold text-sm text-card-foreground">Available Slots for {format(modalSelectedDate, "PPP")}:</h4>
                  <div className="grid grid-cols-3 gap-2">
                    {selectedDoctorForModal.availability[format(modalSelectedDate, "yyyy-MM-dd")].map(slot => (
                      <Button 
                        ...
                        className={modalSelectedTime === slot ? "bg-primary text-primary-foreground" : "hover:bg-accent/10"}
                      >
                        {slot}
                      </Button>
                    ))}
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setIsTimeSlotModalOpen(false)} className="hover:bg-muted/50">Cancel</Button>
              <Button onClick={handleModalBookingConfirm} disabled={!modalSelectedTime} className="bg-primary hover:bg-primary/90 text-primary-foreground">Confirm Appointment</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      )}
