...
import { Stethoscope, Loader2 } from 'lucide-react'; // Assuming Loader2 for loading state
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Error is handled by the useEffect for authError
  };
      ...
      <Card className="w-full max-w-md shadow-xl">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                required
                minLength={isSignUpMode ? 6 : undefined} // Supabase default min password length
                className="rounded-md"
              />
            </div>
            {formError && <p className="text-sm text-destructive">{formError}</p>}
            <Button type="submit" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground rounded-md" disabled={loading}>
          ...
          <div className="mt-6">
            ...
            <Button
              variant="outline"
              className="w-full mt-4 rounded-md flex items-center justify-center gap-2 border-input hover:bg-accent/50"
        ...
        <CardFooter className="flex justify-center">
          <Button variant="link" onClick={() => { setIsSignUpMode(!isSignUpMode); setFormError(null); }} className="text-sm">
            {isSignUpMode
